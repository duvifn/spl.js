 ...
 * Shared Worker
 */
 tape('shared worker large inserts', async t => {

    t.plan(2);
    const spl = await await SPL(undefined, {
        sharedWorker: true
    });
    const db = spl.db();
    const ii = 10000;
...
// TODO: firefox: Dynamic module import is disabled or not supported in this context
tape('shared worker extensions', async t => {
    ...
    const spl = await SPL(extensions, {sharedWorker: true});
